...
crawlFile foreignDeps mvar pkg src docsStatus authorizedForKernelCode expectedName path =
  do
    ...
    case Parse.fromByteString (Parse.Package pkg) bytes of
      Right modul@(Src.ImplementationModule (Just (A.At _ actualName)) _ _ _ _ _ _ _ _ _ _ _) | expectedName == actualName ->
        do
      ...
      Right modul@(Src.SignatureModule (A.At _ actualName) _ _ _ _) | expectedName == actualName ->
        do
          let imports = Src.getModuleImports modul
          deps <- crawlImports foreignDeps mvar pkg authorizedForKernelCode src (fmap snd imports)
          return (Right (SLocal docsStatus deps modul))
...
getDepHome fi =
  case fi of
    ForeignSpecific (I.ImplementationInterface pkg _ _ _ _ _) -> Just pkg
