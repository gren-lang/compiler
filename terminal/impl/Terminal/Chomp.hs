...
chomp maybeIndex strings args flags =
  let (Chomper flagChomper) =
   ...
   in flagChomper (toSuggest maybeIndex) (toChunks strings) ok err
...
chompExactly suggest chunks (Chomper chomper) =
  let ok s cs value =
      ...
      err s argError =
        (s, Left argError)
   in chomper suggest chunks ok err
...
chompMultiple suggest chunks (Chomper chomper) parser =
  let err s1 argError =
        (s1, Left argError)
   in chomper suggest chunks (chompMultipleHelp parser []) err
...
chompRequiredArgs numChunks args =
  case args of
    ...
    Required funcArgs argParser ->
      do
        func <- chompRequiredArgs numChunks funcArgs
...
chompArg numChunks parser@(Parser singular _ _ _ toExamples) =
  Chomper $ \suggest chunks ok err ->
    case chunks of
      [] ->
        let newSuggest = makeSuggestion suggest (suggestArg parser numChunks)
            theError = ArgMissing (Expectation singular (toExamples ""))
         in err newSuggest theError
      Chunk index string : otherChunks ->
        case tryToParse suggest parser index string of
        ...
        chunk@(Chunk index string) : rest ->
          if List.isPrefixOf flagPrefix string
            then succeed (Definitely index (deprefix string)) rest
              ...
              if string /= loneFlag
                then findFlagHelp (chunk : revPrev) loneFlag flagPrefix rest
                else case rest of
                  [] ->
                    succeed DefNope []
  ...
  case unknownFlags of
    ...
    Chunk index string : otherUnknownFlags ->
      if index == targetIndex
        then Just (return (filter (List.isPrefixOf string) (getFlagNames flags [])))
        else suggestFlag otherUnknownFlags flags targetIndex
...
instance Functor (Chomper x) where
  fmap func (Chomper chomper) =
    Chomper $ \i w ok err ->
      let ok1 s1 cs1 value =
            ok s1 cs1 (func value)
       in chomper i w ok1 err
...
instance Applicative (Chomper x) where
  ...
  (<*>) (Chomper funcChomper) (Chomper argChomper) =
    Chomper $ \s cs ok err ->
      let ok1 s1 cs1 func =
            let ok2 s2 cs2 value =
                  ok s2 cs2 (func value)
             in argChomper s1 cs1 ok2 err
       in funcChomper s cs ok1 err
